...
interface SalesReturn extends Sale {
    returnCode: string;
    paidPayment: number;
    due: number;
...
export default function SalesReturnsListPage() {
    ...
    async function fetchData() {
        try {
            ...
            const fetchedReturns = allSales
                .filter(sale => sale.status === 'Return')
                .map((sale, index) => {
                     const paidPayment = sale.total;
                     const due = 0;
                     let paymentStatus: 'Paid' | 'Unpaid' | 'Partial' = 'Paid';
                    return {
                        ...sale,
                        returnCode: `RT-${sale.orderId.split('-')[1] || String(index + 1).padStart(4, '0')}`,
  ...
  const summary = useMemo(() => ({
      ...
      totalInvoicesAmount: filteredReturns.reduce((acc, item) => acc + item.total, 0),
  ...
  const tableTotals = useMemo(() => ({
      total: filteredReturns.reduce((acc, item) => acc + item.total, 0),
  ...
  const getTableData = () => filteredReturns.map(item => ({
    'Sales Date': item.date,
    'Sales Code': item.orderId,
  ...
  const handleExport = (format: 'csv' | 'excel') => {
    ...
    link.setAttribute("download", `sales-returns-export.${format === 'excel' ? 'csv' : format}`);
  ...
  const handlePdfExport = () => {
    ...
    doc.autoTable({
        head: [['Sales Date', 'Sales Code', 'Return Code', 'Customer', 'Total', 'Paid', 'Due', 'Payment Status']],
  ...
  return (
      <div className="flex flex-col gap-6 print:gap-0">
        <div className="print:hidden">
            <h1 className="text-2xl font-bold">Sales Returns List</h1>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 print:hidden">
            <Card className="bg-[#17a2b8] text-white">
             ...
             <Card className="bg-[#dc3545] text-white">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Total Sales Return Due</CardTitle>
        ...
        <Card className="print:border-none print:shadow-none">
            <CardHeader className="flex items-center justify-between print:hidden">
                <CardTitle className="text-lg">Sales Returns List</CardTitle>
                <Button asChild>
