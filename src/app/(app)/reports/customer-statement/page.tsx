...
import { CalendarIcon, User, RefreshCw, Printer, FileText, Image as ImageIcon, Receipt } from 'lucide-react';
...
export default function CustomerStatementReportPage() {
    ...
    const [reportData, setReportData] = useState<{ customer: Customer, transactions: Transaction[] } | null>(null);
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            if (customerData) {
                ...
                if (customerData.balanceDate) {
                  combined.unshift({
                      id: 'opening', date: customerData.balanceDate, type: 'Opening Balance', reference: 'N/A', description: 'Opening Balance', debit: openingBalance >= 0 ? openingBalance : 0, credit: openingBalance < 0 ? -openingBalance : 0, balance: 0 
                  })
                }
                ...
                setReportData({
                 ...
                 toast({ variant: 'destructive', title: 'Error', description: 'Could not find selected customer data.' });
            }
        } catch (error) {
            console.error("Failed to generate report:", error);
            toast({ variant: 'destructive', title: 'Error', description: 'Failed to generate customer statement.' });
        } finally {
            setGenerating(false);
        }
    };
    ...
    const handlePdfExport = () => {
        ...
        const doc = new jsPDF();
        const { customer, transactions } = reportData;
        ...
        doc.text('CUSTOMER STATEMENT', 105, 55, { align: 'center' });

        doc.setFontSize(12);
        doc.setTextColor(0, 0, 0);
        doc.text(`Customer: ${customer.name}`, 105, 68, { align: 'center' });
        doc.setFontSize(10);
    ...
    const handleImageExport = () => {
        ...
        if (reportElement) {
            html2canvas(reportElement, { scale: 2 }).then((canvas) => {
                ...
                link.href = image;
                link.download = `customer-statement-${reportData?.customer.name}.png`;
                link.click();
            });
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="print:hidden">
                <Card>
                    <CardHeader>
                        <CardTitle>Customer Statement Report</CardTitle>
                        <CardDescription>Select filters to generate a customer statement.</CardDescription>
                    </CardHeader>
                    <CardContent>
                 ...
                 <div className="print:hidden">
                    <Card>
                        <CardContent className="p-8 text-center text-muted-foreground">
                            ...
                            <p>Select a customer and date range to view their transaction history.</p>
                        </CardContent>
                    </Card>
                </div>
            )}
