...
import { Calendar as CalendarIcon, Trash2, Loader2, Save } from 'lucide-react';
...
export default function EditInvoicePage() {
      ...
      const saleData = await getSaleById(id);
       if (saleData) {
        ...
        setOrderItems(itemsForOrder);
         ...
         router.push('/accounting/receivable/invoices');
       }
     } catch(e) {
  ...
  const handleUpdateSale = async () => {
    if (!id || orderItems.length === 0) {
        toast({ variant: 'destructive', title: 'Error', description: 'Please add at least one item to the invoice.' });
        return;
    }
    setIsSaving(true);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Edit Invoice: {saleId}</CardTitle>
        <CardDescription>Update invoice details below.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <div className="grid gap-2">
            <Label htmlFor="sales-date">Invoice Date</Label>
