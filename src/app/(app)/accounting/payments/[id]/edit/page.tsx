...
import { Calendar as CalendarIcon, Save, Loader2, UserPlus } from 'lucide-react';
...
import { getPaymentById, updatePayment, type Payment } from '@/services/payments';
...
export default function EditPaymentPage() {
  ...
  const fetchPayment = useCallback(async () => {
    ...
    try {
        setLoading(true);
        const [fetchedCustomers, paymentData] = await Promise.all([
             getCustomers(),
             getPaymentById(id)
        ]);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardHeader>
            ...
            <CardDescription>Update the details for the payment received.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-6">
            <FormField
